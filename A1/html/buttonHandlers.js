...
var prevuserRequestObj
var transposedByNSemitones = 0;
...
function handleSubmitButton() {
  ...
  if (userText && userText !== '') {
    ...
    prevuserRequestObj = userRequestObj
    ...
    xhttp.onreadystatechange = function () {
      if (this.readyState == 4 && this.status == 200) {
        ...
        if (responseObj.songLines) {
          ...
          currentSongLines = song.songLines
          transposedByNSemitones = 0 //reset transpose to no-transpose
          parseChordProFormat(currentSongLines, transposedByNSemitones)
        }
...
function handleTransposeUp() {//doesn't interact with the textbox. Otherwise, very similar to handleSubmitButton
  //USES older-style XMLHttpRequest which we will replacde later with fetch()

  //uses the stored request object
  let userRequestJSON = JSON.stringify(prevuserRequestObj)
  ...
  xhttp.onreadystatechange = function () {
    if (this.readyState == 4 && this.status == 200) {
      ...
      if (responseObj.songLines) {
        ...
        transposedByNSemitones = transposedByNSemitones + 1 //reset transpose to no-transpose
        parseChordProFormat(currentSongLines, transposedByNSemitones)
      }
...
function handleTransposeDown() {//same code as handleTransposeUp, however decrements the transpose amount instead
  //USES older-style XMLHttpRequest which we will replacde later with fetch()
  ...
  xhttp.onreadystatechange = function () {
    if (this.readyState == 4 && this.status == 200) {
      ...
      if (responseObj.songLines) {
        song.songLines = responseObj.songLines
        currentSongLines = song.songLines
        transposedByNSemitones = transposedByNSemitones - 1
        parseChordProFormat(currentSongLines, transposedByNSemitones)
      }
